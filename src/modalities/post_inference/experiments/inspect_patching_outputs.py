...
class PatchingInspector:
    ...
    def load_few_shot_examples(self, *, n_shots=3):
        ...
        self.few_shot_examples = []
        for ex in dataset:
            self.few_shot_examples.append({"question": ex["question"], "answer": ex["answer"]})
    ...
    def generate_with_patching(self, *, prompt, patch_targets, scale_factor, max_new_tokens=256):
        ...
        response = self.patcher.tokenizer.decode(outputs[0][inputs["input_ids"].shape[1] :], skip_special_tokens=True)
    ...
    def inspect_layer(
        ...
        *,
        ...
        print("Identifying significant neurons...")
        ...
        matching_layers = {k: v for k, v in patch_targets.items() if layer_pattern.lower() in k.lower()}
        ...
        for idx, example in enumerate(tqdm(dataset, desc="Processing examples")):
            ...
            example_result = {"question": question, "expected_answer": expected_answer, "generations": {}}

            for scale in scale_factors:
                generation = self.generate_with_patching(
                    prompt=prompt, patch_targets=matching_layers, scale_factor=scale, max_new_tokens=max_new_tokens
                )
        ...
        accuracy_by_scale = {}
        for scale in scale_factors:
            correct = sum(1 for ex in results["examples"] if ex["generations"][f"scale_{scale}"]["is_correct"])
            accuracy_by_scale[scale] = correct / len(results["examples"])

        results["summary"] = {"accuracy_by_scale": accuracy_by_scale}
        ...
        *,
        ...
        for example in tqdm(dataset, desc="Comparing"):
            ...
            gen_a = self.generate_with_patching(
                prompt=prompt, patch_targets=matching_layers, scale_factor=scale_a, max_new_tokens=max_new_tokens
            )

            gen_b = self.generate_with_patching(
                prompt=prompt, patch_targets=matching_layers, scale_factor=scale_b, max_new_tokens=max_new_tokens
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="inspect", choices=["inspect", "compare"])
    parser.add_argument(
        "--scales", type=float, nargs="+", default=[0.0, 1.0, 1.2], help="Scale factors to test (inspect mode)"
    )
    parser.add_argument("--scale_a", type=float, default=0.0, help="First scale for comparison (compare mode)")
    parser.add_argument("--scale_b", type=float, default=1.2, help="Second scale for comparison (compare mode)")
    ...
    parser.add_argument(
        "--n_shots", type=int, default=0, help="Number of few-shot examples (0=zero-shot, 3=three-shot, 5=five-shot)"
    )

    args = parser.parse_args()

    inspector = PatchingInspector(model_path=args.model, h5_path=args.h5_path)
    ...
    if args.mode == "inspect":
        ...
        print("=" * 80)
        print("Mode: Inspect multiple scales")
