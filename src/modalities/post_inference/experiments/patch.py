        ...
        for i in tqdm(range(0, len(prompts), batch_size), desc="GSM8K"):
            ...
            with torch.no_grad():
                outputs = self.model.generate(
                    **inputs, max_new_tokens=256, do_sample=False, pad_token_id=self.tokenizer.pad_token_id
                )
...
class PatchingExperiment:
    ...
    def run(
        ...
        *,
        ...
        all_results = {"baseline": {"gsm8k": baseline_gsm8k, "hellaswag": baseline_hellaswag}, "experiments": []}
        ...
        for d_threshold in d_thresholds:
            for std_threshold in std_thresholds:
                ...
                print(
                    f"Found {n_neurons} significant neurons (|d|>{d_threshold}, std>{std_threshold}) across {len(patch_targets)} layers"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="/raid/s3/opengptx/mfrey/instruct/hf_model", help="Path to model")
    parser.add_argument(
        "--h5_path",
        type=str,
    ...
    parser.add_argument("--n_eval_samples", type=int, default=100, help="Number of evaluation samples per benchmark")
