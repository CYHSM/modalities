...
class CorrectIncorrectMathExperiment:
    ...
    def capture_with_generation(self, *, prompts, expected_answers, max_new_tokens=256, batch_size=8):
        ...
        for i in tqdm(range(0, len(prompts), batch_size)):
            ...
            for prompt, expected in zip(batch_prompts, batch_expected):
                ...
                activations, texts = self.capture.capture_generation(prompt, max_new_tokens=1, layers_to_capture=None)
    ...
    def compute_statistics_with_summary(
        self, *, correct_activations, incorrect_activations, test="welch", correction_method="fdr_bh"
    ):
    ...
    def run(self, *, n_samples=500, max_new_tokens=256, batch_size=8, test="welch", correction_method="fdr_bh"):
        output_path = (
            Path("/raid/s3/opengptx/mfrey/activation/experiments")
            / f"{self.model_path.replace('/', '_')}_correct_vs_incorrect"
        )
        ...
        (
            ...
            incorrect_texts,
            incorrect_prompts,
        ) = self.capture_with_generation(
            prompts=prompts, expected_answers=expected_answers, max_new_tokens=max_new_tokens, batch_size=batch_size
        )
        ...
        n_significant_corr = sum(np.sum(layer_statistics[layer]["significant"]) for layer in layer_statistics)
        total_dims = sum(len(layer_statistics[layer]["p_values"].flatten()) for layer in layer_statistics)
        ...
        print(
            f"FDR-corrected significant (p<0.05): {n_significant_corr}/{total_dims} ({100*n_significant_corr/total_dims:.2f}%)"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--correction", type=str, default="fdr_bh", choices=["fdr_bh", "bonferroni", "fdr_by"])
