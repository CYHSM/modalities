...
def plot_layer_sequence(*, results, output_path):
    ...
    ax.plot(x_pos, gsm8k, marker='o', linewidth=2, markersize=3,
            label='GSM8K', color='#4575b4', alpha=0.8)
    ax.plot(x_pos, hellaswag, marker='s', linewidth=2, markersize=3,
            label='HellaSwag', color='#d73027', alpha=0.8)
    
    ax.axhline(y=baseline_gsm8k, color='#4575b4', linestyle='--',
               linewidth=1.5, label='GSM8K Baseline', alpha=0.5)
    ax.axhline(y=baseline_hellaswag, color='#d73027', linestyle='--',
               linewidth=1.5, label='HellaSwag Baseline', alpha=0.5)
    ...
    ax.set_xticklabels([names[i] for i in range(0, len(names), max(1, len(names)//30))],
...
def plot_layer_sequence_deltas(*, results, output_path):
    ...
    ax.plot(x_pos, gsm8k_deltas, marker='o', linewidth=2, markersize=3,
            label='GSM8K Δ', color='#4575b4', alpha=0.8)
    ax.plot(x_pos, hellaswag_deltas, marker='s', linewidth=2, markersize=3,
            label='HellaSwag Δ', color='#d73027', alpha=0.8)
...
def main():
    ...
    parser.add_argument("--positive_only", action="store_true",
                       help="Only show layers with positive GSM8K delta")
    ...
    plot_layer_impact(results=results, output_path=output_path, 
                     top_n=args.top_n, metric='gsm8k', positive_only=args.positive_only)
    plot_layer_impact(results=results, output_path=output_path,
                     top_n=args.top_n, metric='hellaswag', positive_only=args.positive_only)
