...
def run_evaluation(checkpoint_path, output_file, device="cuda:7", batch_size=8, limit=0.01):
    ...
    cmd = [
        ...
        "hf",
        "--model_args",
        f"pretrained={checkpoint_path},trust_remote_code=True",
        "--tasks",
        tasks,
        "--device",
        device,
...
def plot_results(df):
    ...
    possible_metrics = {
        ...
        "gsm8k_cot": [
            "gsm8k_cot_exact_match",
            "gsm8k_cot_exact_match,strict-match",
            "gsm8k_cot_exact_match,flexible-extract",
        ],
    ...
    for i, (task, metric) in enumerate(key_metrics.items()):
        ...
        for subset in df["subset"].unique():
            ...
            if not data.empty and metric in data.columns:
                axes[i].plot(data["step"], data[metric], "o-", label=subset.replace("full_", ""), linewidth=2)
...
def main():
    ...
    parser.add_argument(
        "--base-dir", default="/raid/s3/opengptx/mfrey/instruct/checkpoints_sft", help="Base directory with checkpoints"
    )
    parser.add_argument(
        "--results-dir", default="/raid/s3/opengptx/mfrey/cp_analysis/benchmark_cps", help="Directory to save results"
    )
    ...
    parser.add_argument(
        "--subsets",
        nargs="+",
        default=["full_general", "full_math", "full_code", "full_mix"],
        help="Training subsets to evaluate",
    )
    parser.add_argument(
        "--steps", nargs="+", type=int, default=list(range(5000, 100000, 5000)), help="Checkpoint steps to evaluate"
    )
