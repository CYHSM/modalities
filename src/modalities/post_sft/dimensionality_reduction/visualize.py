...
def plot_trajectories(coords, dataset_names, checkpoint_numbers, dims=(0,1), figsize=(12, 8), arrow_stride=2):
    ...
    for dataset in dataset_names:
        ...
        ax.quiver(X[::arrow_stride], Y[::arrow_stride], U[::arrow_stride], V[::arrow_stride], 
                  angles='xy', scale_units='xy', scale=1, 
                  color=color, alpha=0.9, width=0.004, zorder=2)
        ...
        ax.scatter(trajectory[-1, 0], trajectory[-1, 1], 
                  color=color, s=400, marker='*', alpha=1.0,
                  edgecolors='white', linewidth=2, zorder=0,
...
def plot_contour_with_trajectories(coords, benchmark_scores, metric_name, 
                                 dataset_names, checkpoint_numbers, dims=(0,1), figsize=(14, 10)):
    ...
    if len(valid_scores) > 3:
        ...
        zi = griddata((valid_coords[:, 0], valid_coords[:, 1]), valid_scores, 
                     (xi, yi), method='linear', fill_value=np.nan)
...
def plot_benchmark_progression(benchmark_scores, dataset_names, checkpoint_numbers, 
                               smoothing_window=4, figsize=(16, 12)):
    metrics = {
    ...
    for idx, (metric_key, metric_title) in enumerate(metrics.items()):
        ...
        for dataset in dataset_names:
            ...
            if valid_mask.any():
                ...
                ax.plot(valid_checkpoints, valid_scores, 
                       color=color, linewidth=1.5, marker='',
                       linestyle='--', alpha=0.3)
                
                if len(valid_scores) >= smoothing_window:
                    smooth_scores = pd.Series(valid_scores).rolling(
                        window=smoothing_window, center=True, min_periods=1
                    ).mean()
                    ax.plot(valid_checkpoints, smooth_scores,
...
def main():
    ...
    for dims in dim_pairs:
        ...
        for metric_name in benchmark_scores.keys():
            fig = plot_contour_with_trajectories(coords, benchmark_scores, metric_name,
                                                dataset_names, checkpoint_numbers, dims)
            if fig:
                plt.savefig(output_dir / f'contour_smoothed_{metric_name}_dims_{dims[0]}_{dims[1]}.png', 
