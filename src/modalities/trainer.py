...
class RandomPonderScheduler:
    def __init__(self, model: torch.nn.Module, min_weight: float = -0.2, max_weight: float = 0.2, seed: int = 42):
...
class AsymmetricPonderScheduler:
    def __init__(
        self, model: torch.nn.Module, steps_per_cycle: int,
...
def create_ponder_scheduler(model, scheduler_type: str, config_weight: float, num_target_steps: int, global_rank: int):
    """Factory function — add new scheduler types here."""
    if scheduler_type == "constant":
        return ConstantPonderScheduler(model=model, constant_value=config_weight)
    elif scheduler_type == "random":
        return RandomPonderScheduler(model=model, min_weight=1, max_weight=1, seed=42 + global_rank)
    elif scheduler_type == "asymmetric":
        return AsymmetricPonderScheduler(
            model=model, steps_per_cycle=10,
...
class Trainer:
    ...
    def _train_batch(
        self, batch, model, optimizer, scheduler, loss_fun, micro_batch_id,
        ...
        num_train_steps_done = Trainer._get_num_train_steps_done(micro_batch_id, self.gradient_acc_steps)
    ...
    def train(
        ...
        ponder_scheduler = create_ponder_scheduler(
            model, scheduler_type, config_weight, self.num_target_steps, self.global_rank,
        )
        ...
        for _, (micro_batch_id, batch) in zip(range(num_batches_todo), enumerate(train_loader)):
            ...
            (
                ...
                batch=batch, model=model, optimizer=optimizer, scheduler=lr_scheduler,
                loss_fun=loss_fun, micro_batch_id=micro_batch_id,
            ...
            if training_progress.num_seen_steps_total % training_log_interval_in_steps == 0 and step_performed:
                ...
                # --- Reduce total loss (same as before) ---
    ...
    @staticmethod
    def _publish_progress(progress_publisher, num_train_steps_done, dataloader_tag):
    ...
    @staticmethod
    def _publish_evaluation_result(evaluation_result_publisher, evaluation_result):
