"""Custom trainer implementation with evaluation callbacks and alignment support."""
import logging
import math
import os
...
from modalities.post_distill.aligned_sft_trainer import AlignedSFTTrainer, AlignedSFTConfig
from torch.optim.lr_scheduler import LambdaLR
from transformers import TrainerCallback
from trl import SFTTrainer

from modalities.post_distill.config import EvaluationConfig, TrainingConfig
...
class CustomSFTTrainer(AlignedSFTTrainer, SFTTrainer):
    """Custom SFT Trainer with model saving enhancements and optional alignment."""
    ...
    def __init__(self, source_model_path: str, *args, **kwargs):
        ...
        if self.args.teacher_model_name_or_path:
            self._setup_teacher_model(self.args)
...
def create_sft_config(training_config: TrainingConfig, eval_config, alignment_config) -> AlignedSFTConfig:
    """Create AlignedSFTConfig from TrainingConfig and AlignmentConfig."""
    config = AlignedSFTConfig(
        ...
        fp16=False,
        torch_compile=False,
        teacher_model_name_or_path=alignment_config.teacher_model_name_or_path,
        alignment_loss_type=alignment_config.alignment_loss_type,
        alignment_weight=alignment_config.alignment_weight,
        teacher_gpu=alignment_config.teacher_gpu,  # Added this
        teacher_model_init_kwargs=alignment_config.teacher_model_init_kwargs,
        student_layers_to_align=list(range(-2, -33, -1)),
        teacher_layers_to_align=list(range(-2, -33, -1)),
    )
...
def setup_trainer(
    ...
    sft_config = create_sft_config(training_config, eval_config, alignment_config)
