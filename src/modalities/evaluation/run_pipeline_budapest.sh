...
EXPERIMENTS_DIR="/leonardo_scratch/large/userexternal/mfrey000/experiments_budapest"
...
echo "=== 0. Building list of checkpoints to convert (LAST CHECKPOINT ONLY) ==="
rm -f "$CONV_LIST"
needs_conversion=0
for run_dir in "${EXPERIMENTS_DIR}"/*/; do
    ...
    # Find the checkpoint with seen_steps_18360 (the absolute last one)
    last_ckpt_dir=$(ls -d "${run_dir}"eid_*-seen_steps_18360-*/ 2>/dev/null | head -n 1)
    if [ -n "$last_ckpt_dir" ]; then
...
echo "=== 2. Finding checkpoints to evaluate (LAST CHECKPOINT ONLY) ==="
rm -f "$LIST_FILE"
found=0
for run_dir in "${EXPERIMENTS_DIR}"/*/; do
    [ -d "$run_dir" ] || continue
