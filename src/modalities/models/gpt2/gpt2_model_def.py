...
class GatedMemoryUnit(nn.Module):
    ...
    def reset_parameters(self):
        """Reset the parameters of the gated memory unit."""
...
class AdaptiveRecursiveBlock(nn.Module):
    ...
    Optionally, a memory bank provides extra context at each step via
    gated cross-attention over learned key-value slots.
    """
    ...
    def forward(self, x: torch.Tensor, token_ids: torch.Tensor = None) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        B, T, D = x.shape
        device = x.device
        ...
        for step in range(self.max_loops):
            ...
            # 5) Early exit when all tokens have halted
        ...
        local_s = step_metrics.get("local_gate", torch.zeros(self.max_loops, device=device)).mean()
        global_s = step_metrics.get("global_gate", torch.zeros(self.max_loops, device=device)).mean()
        ...
        layer_metrics = {
            ...
            "step_halt_probs": step_metrics.get("halt_prob", torch.zeros(self.max_loops, device=device)),
