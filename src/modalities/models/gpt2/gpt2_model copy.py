...
class GPT2LLM(NNModel):
    ...
    def forward_impl(self, inputs: torch.Tensor) -> dict[str, torch.Tensor] | torch.Tensor:
        ...
        if self.use_adaptive:
            # Average ponder cost per layer (depth-independent!)
            ...
            # Apply weight to average (now transferable across model depths)
            weighted_ponder_loss = (avg_ponder_cost * self.adaptive_config.ponder_penalty_weight).to(logits.dtype)
            
            return {
                "logits": logits,
                "ponder_loss": weighted_ponder_loss,
                "ponder_cost_unweighted": total_ponder_cost,  # Total across all layers (for monitoring)
                "expected_steps": avg_ponder_cost,  # Avg per layer (this is what you weight)
            }
