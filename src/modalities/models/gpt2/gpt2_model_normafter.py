...
class AdaptiveRecursiveBlock(nn.Module):
    ...
    def __init__(
        ...
        layer_type: str = "dual",
        inter_loop_norm: Optional[nn.Module] = None,
    ):
        super().__init__()
    ...
    def forward(
        ...
        if self.has_loop_path:
            ...
            for step in range(self.max_loops):
                ...
                h_loop = self.inter_loop_norm(h_loop)
    ...
    def __init__(
        ...
        for layer_idx in range(n_layer):
            if self.use_adaptive:
                ...
                loop_norm = (attention_norm_config.norm_type.value(**dict(attention_norm_config.config))if l_type in ["loop", "dual"] else None)
