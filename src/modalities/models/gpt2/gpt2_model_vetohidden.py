...
class ValueEmbeddingBank(nn.Module):
    ...
    def __init__(
        ...
        self.embeds = nn.ModuleDict({
            str(i): nn.Embedding(vocab_size, n_embd)
            for i in self.active_layers
        })
        self.gates = nn.ModuleDict({
            str(i): nn.Linear(gate_dim, n_embd, bias=True)
            for i in self.active_layers
        })
        self.reset_parameters()
    ...
    def forward(
        ...
        Args:
            ...
            hidden:    (B, T, D) current hidden state, used for gating
            layer_idx: which layer is calling

        Returns:
            (B, T, n_embd) gated value embedding to add as residual
        """
        key = str(layer_idx)
        ve = self.embeds[key](token_ids)                                # (B, T, n_embd)
...
class AdaptiveRecursiveBlock(nn.Module):
    ...
    Optionally includes:
      ...
      - Value embeddings: token-indexed static capacity injection
    """
    ...
    def forward(
        ...
        for step in range(self.max_loops):
            ...
            # 2) Transformer block with learnable scale
            scale = F.softplus(self.loop_scales[step])
            h_prev = h
            h = self.block(h_enriched, scale=scale)

            # 3) Value embedding — static capacity injection
            if has_ve:
                h = h + self.ve_bank(token_ids, h, self.layer_idx)
