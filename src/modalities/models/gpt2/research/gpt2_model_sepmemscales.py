...
class AdaptiveRecursiveBlock(nn.Module):
    ...
    Now with per-loop control over:
    - loop_scales: how much each loop contributes overall
    - local_mem_scales: how much local memory to use per loop
    - global_mem_scales: how much global memory to use per loop
    """
    def __init__(
        self,
        block: GPT2Block,
        ...
        # Per-loop scales (all start small via softplus)
        self.loop_scales = nn.Parameter(torch.full((self.max_loops,), -7.0))
        self.local_mem_scales = nn.Parameter(torch.full((self.max_loops,), -7.0))
        self.global_mem_scales = nn.Parameter(torch.full((self.max_loops,), -7.0))

        # Memory injection projections
    ...
    def forward(self, x: torch.Tensor) -> tuple:
        ...
        for step in range(self.max_loops):
            ...
            # ========== 2. PER-LOOP MEMORY INJECTION ==========
            local_scale = F.softplus(self.local_mem_scales[step])
            global_scale = F.softplus(self.global_mem_scales[step])
            
            h_enriched = (
                h
                + local_scale * self.local_mem_proj(local_mem)
                + global_scale * self.global_mem_proj(global_mem)
            )

            # ========== 3. TRANSFORMER BLOCK ==========
            block_scale = F.softplus(self.loop_scales[step])
            h_prev = h
            h_new = self.block(h_enriched, scale=block_scale)
        ...
        return (
            ...
            halt_probs_stacked,
            F.softplus(self.local_mem_scales).detach(),   # [max_loops] tensor
            F.softplus(self.global_mem_scales).detach(),  # [max_loops] tensor
        )
...
class GPT2LLM(NNModel):
    def __init__(
        ...
        weight_decay_groups = {
            ...
            "layernorm": [
                ...
                ".mem_norm",
                ".loop_scales",
                ".local_mem_scales",   # Changed from .local_mem_scale
                ".global_mem_scales",  # Changed from .global_mem_scale
    ...
    def forward_impl(self, inputs: torch.Tensor) -> dict[str, torch.Tensor] | torch.Tensor:
        ...
        for layer_key in sorted_keys:
            ...
            if self.use_adaptive:
                h, cost, cos_sim, scales, halt_probs, local_mem_scales, global_mem_scales = layer_module(h)
                cost_mean = cost.mean()
                ...
                per_layer_mem_scales.append((local_mem_scales, global_mem_scales))  # Now tuple of tensors
        ...
        if self.use_adaptive:
            avg_ponder_cost = total_ponder_cost / num_adaptive_layers if num_adaptive_layers > 0 else torch.tensor(
                0.0, dtype=h.dtype, device=device)
            normalized_steps = (avg_ponder_cost - 1.0) / (
                self.adaptive_config.max_loops - 1.0) if self.adaptive_config.max_loops > 1 else torch.tensor(
                0.0, dtype=h.dtype, device=device)
            weighted_ponder_loss = (normalized_steps * self.adaptive_config.ponder_penalty_weight).to(logits.dtype)
            layer_costs_tensor = torch.stack(per_layer_ponder_costs) if per_layer_ponder_costs else torch.tensor(
                [], device=device)
            layer_sims_tensor = torch.stack(per_layer_cos_sims) if per_layer_cos_sims else torch.tensor(
                [], device=device)
            local_mem_scales_tensor = torch.stack([s[0] for s in per_layer_mem_scales])  # [n_layer, max_loops]
            global_mem_scales_tensor = torch.stack([s[1] for s in per_layer_mem_scales])  # [n_layer, max_loops]
            ...
            return {
                ...
                "loop_scales": torch.stack(per_layer_loop_scales) if per_layer_loop_scales else torch.tensor(
                    [], device=device),
                "halt_probs": torch.stack(per_layer_halt_probs) if per_layer_halt_probs else torch.tensor(
                    [], device=device),
                "local_mem_scales": local_mem_scales_tensor,   # [n_layer, max_loops]
                "global_mem_scales": global_mem_scales_tensor, # [n_layer, max_loops]
            }
