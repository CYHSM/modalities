...
class MemoryBankRegistry(nn.Module):
    def __init__(self, n_layer: int, n_embd: int, num_local_slots: int = 1024, num_global_slots: int = 512):
        ...
        # --- MEMORY STORAGE ---
        # Local: Specific to layer execution logic
        ...
        # Global: Universal facts/concepts
        ...
        # Separate Norms for Keys (Queries are normed by the Block now)
        self.k_norm_local = nn.LayerNorm(n_embd)
        self.k_norm_global = nn.LayerNorm(n_embd)

    def forward(self, q_local: torch.Tensor, q_global: torch.Tensor, layer_idx: int):
        # 1. Local Lookup
        # Use q_local vs local_keys
        k_loc = self.k_norm_local(self.local_keys[layer_idx])
        v_loc = self.local_values[layer_idx]
        
        attn_loc = F.softmax(torch.einsum('btd,sd->bts', q_local, k_loc) * self.scale, dim=-1)
        out_loc = torch.einsum('bts,sd->btd', attn_loc, v_loc)
        
        # 2. Global Lookup
        # Use q_global vs global_keys
        k_glob = self.k_norm_global(self.global_keys)
        v_glob = self.global_values
        
        attn_glob = F.softmax(torch.einsum('btd,sd->bts', q_global, k_glob) * self.scale, dim=-1)
        out_glob = torch.einsum('bts,sd->btd', attn_glob, v_glob)
        
        return out_loc, out_glob
...
class AdaptiveRecursiveBlock(nn.Module):
    def __init__(
        ...
        self.q_norm_local_steps = nn.ModuleList([nn.LayerNorm(n_embd) for _ in range(self.max_loops)])
        self.q_norm_global_steps = nn.ModuleList([nn.LayerNorm(n_embd) for _ in range(self.max_loops)])

        # Memory injection - LOCAL
        self.local_mem_proj = nn.Linear(n_embd, n_embd, bias=False)
        self.local_mem_scale = nn.Parameter(torch.tensor([0.5]))
        
        # Memory injection - GLOBAL
        self.global_mem_proj = nn.Linear(n_embd, n_embd, bias=False)
        self.global_mem_scale = nn.Parameter(torch.tensor([0.5]))
    ...
    def forward(self, x: torch.Tensor) -> tuple:
        ...
        for step in range(self.max_loops):
            # ========== 1. MEMORY RETRIEVAL (CHANGED) ==========
            # Select the specific normalization layer for this step
            q_loc = self.q_norm_local_steps[step](h + x)
            q_glob = self.q_norm_global_steps[step](h + x)
            
            local_mem, global_mem = self.memory_registry(q_loc, q_glob, self.layer_idx)
        ...
        return (
            ...
            self.global_mem_scale.detach().item(),
        )
    ...
    def __init__(
        ...
        weight_decay_groups = {
            ...
            "layernorm": [
                ...
                # Query Norms (Now step-specific lists)
                ".q_norm_local_steps",
                ".q_norm_global_steps",
                # --- CHANGED SECTION END ---
                
                "memory_registry.k_norm_local",
                "memory_registry.k_norm_global",
