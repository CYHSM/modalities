...
class AdaptiveComputationConfig(BaseModel):
    ...
    # --- Softplus Scale Inits (NEW) ---
    # 0.5413 targets a scale of ~1.0, -7.0 targets a scale of ~0.0009
    loop_scale_init: float = 0.5413 
...
class DualPathGate(nn.Module):
    def __init__(
        ...
        # Instantiate empty parameters; initialized safely in reset_parameters
    ...
    def reset_parameters(self):
        ...
        # --- SECURED AGAINST GLOBAL FRAMEWORK OVERRIDES ---
...
class AdaptiveRecursiveBlock(nn.Module):
    def __init__(
        ...
        if self.has_loop_path:
            ...
            # Empty initialization; safely filled in reset_parameters
            self.loop_scales = nn.Parameter(torch.empty((self.max_loops,)))
        ...
        if layer_type == "dual":
            self.dual_gate = DualPathGate(
                n_embd=n_embd,
                init_bias_deep=adaptive_config.deep_gate_init_bias, # Pass the actual bias config
                init_bias_wide=adaptive_config.wide_gate_init_bias, # Pass the actual bias config
                cross_scale_deep_init=adaptive_config.cross_scale_deep_init, # Pass the NEW scale config
                cross_scale_wide_init=adaptive_config.cross_scale_wide_init, # Pass the NEW scale config
            )
            
        # --- ENSURE INITIALIZATION SURVIVES ---
        self.reset_parameters()

    def reset_parameters(self):
        """Secures parameters against framework-level global zeroing out."""
        if self.has_loop_path:
            nn.init.constant_(self.loop_scales, self.config.loop_scale_init)
            
        if self.has_wide_path:
            nn.init.constant_(self.wide_scale, self.config.wide_scale_init)
