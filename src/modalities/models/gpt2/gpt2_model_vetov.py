...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve_contribution=None):
        """
        Args:
            x: input hidden state, shape (B, T, D).
            ve_contribution: optional gated value embedding to add to V,
                             shape (B, T, kv_dim) where kv_dim = n_embd // n_rep.
                             Following nanochat convention: v = v + gate * ve.
        """
        B, T, _ = x.size()
        q, k, v = self.projection(x)

        # Add value embedding to V tensor *before* reshaping into heads,
        # matching nanochat: v = v + gate * ve
        if ve_contribution is not None:
            v = v + ve_contribution
...
class GPT2Block(nn.Module):
    ...
    def forward(self, x, scale=1.0, ve_contribution=None):
        """
        Args:
            x: input hidden state, shape (B, T, D).
            scale: residual scaling factor.
            ve_contribution: optional gated value embedding to add to V inside
                             attention, shape (B, T, kv_dim).
        """
        x = x + scale * self.attn(self.attention_norm(x), ve_contribution=ve_contribution)
...
# ---- Halting state bookkeeping -------------------------------------------------

@dataclass
class HaltingState:
    """Tracks Adaptive Computation Time (ACT) halting variables.

    At each step the router emits halt_prob ∈ [0,1] per token.  We maintain:
      • prob_remain  – probability mass still "alive" per token
      • output       – weighted mixture of per-step hidden states
      • expected_steps – E[step index] under the halting distribution

    After all steps, any leftover mass in prob_remain is dumped onto the
    last hidden state (the "remainder" in Graves 2016).
    """
    ...
    def update(self, h: torch.Tensor, halt_prob: torch.Tensor, step: int):
        """Accumulate one step of ACT.

        Args:
            h:         hidden state at this step, shape (B, T, D).
            halt_prob: per-token halting probability, shape (B, T).
            step:      zero-indexed loop counter.
        """
    ...
    def finalize(self, h_last: torch.Tensor, last_step: int):
        """Dump remaining probability mass onto the final hidden state."""
    ...
    def converged(self) -> bool:
        """True when no token has meaningful remaining mass (max < 1%)."""
        return self.prob_remain.max().item() < 0.01


# ---- Per-step metric accumulator -----------------------------------------------

class StepMetrics:
    """Collects per-step scalars during the adaptive loop, then pads/stacks
    them into fixed-length tensors of shape (max_loops,) for logging."""
    ...
    def log(self, key: str, value: torch.Tensor):
        """Record a scalar value for the current step."""
        self._buffers.setdefault(key, []).append(value.detach())

    def finalize(self) -> dict[str, torch.Tensor]:
        """Pad every recorded series to (max_loops,) and return."""
        out = {}
...
class ValueEmbeddingBank(nn.Module):
    """Token-indexed value embeddings for static knowledge capacity.

    Placed at alternating layers (controlled by every_n_layers).
    Massive parameter count, near-zero FLOPs — just an embedding
    lookup plus a small gating linear.

    Each active layer gets its own embedding table and gate.
    The gate starts near-closed (bias=-3.0 → sigmoid ≈ 0.05) so
    the model can learn to open it gradually during training.
    Output range is (0, 2) * embedding, following the nanochat convention.

    The embedding dimension is kv_dim (= n_embd // n_rep for GQA),
    so the output can be added directly to the V tensor inside attention:
        v = v + gate * ve
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.active_layers = set(range(0, n_layer, every_n_layers))
        self.gate_dim = gate_dim

        # Embed to kv_dim so output matches V projection dimension
        self.embeds = nn.ModuleDict({
            str(i): nn.Embedding(vocab_size, kv_dim)
            for i in self.active_layers
        })
        # Gate is driven by first gate_dim features of the hidden state (n_embd),
        # but outputs kv_dim to match the embedding
        self.gates = nn.ModuleDict({
            str(i): nn.Linear(gate_dim, kv_dim, bias=True)
            for i in self.active_layers
        })
        self.reset_parameters()
    ...
    def has_layer(self, idx: int) -> bool:
        return idx in self.active_layers

    def forward(
        self, token_ids: torch.Tensor, hidden: torch.Tensor, layer_idx: int,
    ) -> torch.Tensor:
        """Compute gated value embedding to be added to V inside attention.

        Args:
            token_ids: (B, T) input token indices
            hidden:    (B, T, D) current hidden state (pre-block), used for gating
            layer_idx: which layer is calling

        Returns:
            (B, T, kv_dim) gated value embedding: gate * ve
                where gate ∈ (0, 2) following nanochat convention
        """
        key = str(layer_idx)
        ve = self.embeds[key](token_ids)                                # (B, T, kv_dim)
        gate = 2.0 * torch.sigmoid(
            self.gates[key](hidden[:, :, :self.gate_dim])
        )                                                                # (B, T, kv_dim) in (0, 2)
        return gate * ve
...
class MemoryBankRegistry(nn.Module):
    """Learned key-value memory with per-layer local slots and shared global slots.

    Each forward call computes cross-attention between `query` and both the
    layer-specific local bank and the shared global bank, returning two
    ...
    def forward(self, query: torch.Tensor, layer_idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        """Returns (local_output, global_output), each shape (B, T, D)."""
        q = self.q_norm(query)
...
class GatedMemoryUnit(nn.Module):
    """Learned gate that controls how much retrieved memory is mixed in.

    Returns (gated_output, gate_mean) where gate_mean is a scalar for logging.
    Initialized with negative bias so the gate starts near-closed.
    """
    ...
    def __init__(self, n_embd: int, init_bias: float = -3.0, frozen_gate: Optional[float] = None):
        ...
        nn.init.constant_(self.gate_net.bias, init_bias)
...
class AdaptiveRouter(nn.Module):
    """Per-token halting decision for Adaptive Computation Time.

    Takes the current hidden state h ∈ R^{B×T×D} and a normalized step
    index t ∈ [0, 1], and outputs a halting probability p ∈ (0, 1) per
    token.  The step index lets the router learn step-dependent behaviour
    (e.g. "almost always continue at step 0, consider halting later").

    Architecture:
        [h ; t] ∈ R^{D+1}  →  Linear  →  σ  →  p ∈ (0,1)
    """
    ...
    def forward(self, h: torch.Tensor, step_normalized: float) -> torch.Tensor:
        """
        Args:
            h:               hidden state, shape (B, T, D).
            step_normalized: scalar in [0, 1] indicating loop progress.
        ...
        logit = self.linear(torch.cat([h, step_feat], dim=-1))  # (B, T, 1)
        return torch.sigmoid(logit).squeeze(-1)                 # (B, T)
...
class AdaptiveRecursiveBlock(nn.Module):
    """Wraps a transformer block with Adaptive Computation Time (ACT).

    On each forward pass the block is applied up to `max_loops` times.
    A learned router decides per-token when to halt; the output is the
    probability-weighted mixture of all intermediate hidden states.

    Optionally includes:
      - Memory bank: learned KV slots with gated cross-attention
      - Value embeddings: token-indexed capacity injected into the V tensor
        inside attention (following nanochat convention)
    """
    ...
    def __init__(
        ...
        ve_bank: Optional[ValueEmbeddingBank] = None,
    ):
        super().__init__()
        self.block = block
        ...
        if memory_registry is not None:
            ...
            self.local_mem_gate = GatedMemoryUnit(
                n_embd, init_bias=-3.0, frozen_gate=adaptive_config.frozen_gate,
            )
            self.global_mem_gate = GatedMemoryUnit(
                n_embd, init_bias=-3.0, frozen_gate=adaptive_config.frozen_gate,
            )

        # Value embeddings (optional)
        self.ve_bank = ve_bank

    # ----- sub-steps (keep the main loop readable) -----

    def _enrich_with_memory(
        self, h: torch.Tensor, metrics: StepMetrics,
    ) -> torch.Tensor:
        """Cross-attend to memory bank and gate the result into h."""
        if self.memory_registry is None:
            return h
        query = self.mem_norm(h)
        local_mem, global_mem = self.memory_registry(query, self.layer_idx)
        local_contrib, l_gate = self.local_mem_gate(h, local_mem)
        global_contrib, g_gate = self.global_mem_gate(h, global_mem)
        metrics.log("local_gate", l_gate)
        metrics.log("global_gate", g_gate)
        return h + local_contrib + global_contrib

    def _compute_relative_change(
        self, h_new: torch.Tensor, h_prev: torch.Tensor,
    ) -> torch.Tensor:
        """Token-level relative change ‖Δh‖ / ‖h_prev‖, shape (B, T)."""
        return (h_new - h_prev).norm(dim=-1) / (h_prev.norm(dim=-1) + 1e-6)
    ...
    def forward(
        ...
        # Check once whether VE applies to this layer
        has_ve = (
            self.ve_bank is not None
            and token_ids is not None
            and self.ve_bank.has_layer(self.layer_idx)
        )
        ...
        for step in range(self.max_loops):
            ...
            # 1) Memory bank enrichment
            h_enriched = self._enrich_with_memory(h, metrics)

            # 2) Compute VE contribution from current hidden state (pre-block)
            #    This will be added to V inside the attention mechanism.
            ve_contribution = None
            if has_ve:
                ve_contribution = self.ve_bank(token_ids, h, self.layer_idx)

            # 3) Transformer block with learnable scale and VE injected into V
            scale = F.softplus(self.loop_scales[step])
            h_prev = h
            h = self.block(h_enriched, scale=scale, ve_contribution=ve_contribution)

            # 4) Router halting decision
            halt_prob = self.router(h, step_normalized=step / step_denom)
            state.update(h, halt_prob, step)

            # 5) Diagnostics (detached — no gradient impact)
            rel_change = self._compute_relative_change(h, h_prev)
            metrics.log("halt_prob", halt_prob.detach().mean())
            ...
            # 6) Early exit when all tokens have halted
            if state.prob_remain.max() < (1 - self.config.halt_threshold):
                break

        # Dump remaining mass
        state.finalize(h, actual_steps)

        # --- Build layer metrics dict ---
        step_metrics = metrics.finalize()

        local_s = step_metrics.get(
            "local_gate", torch.zeros(self.max_loops, device=device),
        ).mean()
        global_s = step_metrics.get(
            "global_gate", torch.zeros(self.max_loops, device=device),
        ).mean()

        layer_metrics = {
            # Per-token aggregates
            "expected_steps": state.expected_steps,         # (B, T)
            # Scalars
            "local_mem_scale": local_s,
            "global_mem_scale": global_s,
            "actual_steps": torch.tensor(float(actual_steps), device=device),
            "residual_mass": state.prob_remain.mean().detach(),
            # Per-step vectors (all padded to max_loops)
            "step_halt_probs": step_metrics.get(
                "halt_prob", torch.zeros(self.max_loops, device=device),
            ),
            "step_changes": step_metrics.get(
                "rel_change", torch.zeros(self.max_loops, device=device),
            ),
            "loop_scales": self.loop_scales.detach(),
            "prob_remain_max": step_metrics.get(
                "prob_remain_max", torch.zeros(self.max_loops, device=device),
            ),
            "prob_remain_mean": step_metrics.get(
                "prob_remain_mean", torch.zeros(self.max_loops, device=device),
            ),
        }

        return state.output, layer_metrics
...
class GPT2LLM(NNModel):
    def __init__(
        ...
        weight_decay_groups = {
            "linear": [
                ...
                ".mem_proj.weight", ".gate_net.weight", ".router.linear.weight",
            ],
            "embedding": [".wte", ".wpe", "ve_bank.embeds"],
            "layernorm": [
                ".attention_norm", ".ffn_norm", ".lm_head_norm", ".mem_norm",
                ".loop_scales", ".gate_net.bias",
                "memory_registry", ".router.linear.bias",
                "ve_bank.gates",
            ],
        }
        ...
        # Compute kv_dim for GQA-aware value embeddings
        n_rep = n_head_q // n_head_kv
        kv_dim = n_embd // n_rep
        ...
        # Value embedding bank (optional)
        # Embeds to kv_dim so output is added directly to V inside attention
        if (
            self.use_adaptive
            and adaptive_config is not None
            and adaptive_config.use_value_embeddings
        ):
            self.ve_bank = ValueEmbeddingBank(
                n_layer=n_layer,
                vocab_size=vocab_size,
                kv_dim=kv_dim,
                n_embd=n_embd,
                gate_dim=adaptive_config.ve_gate_dim,
                every_n_layers=adaptive_config.ve_every_n_layers,
            )
        else:
            self.ve_bank = None
    ...
    # ------------------------------------------------------------------
    # Metrics bag construction (extracted from forward_impl for clarity)
    # ------------------------------------------------------------------
    ...
    def _build_metrics_bag(
        ...
        """Assemble the ponder loss and structured metrics dict from per-layer results.

        Returns:
            weighted_ponder_loss: scalar loss term to add to the main loss.
            metrics_bag:          dict with "scalars", "per_layer_scalars", "per_layer_vectors".
        """
        ...
        per_layer_vectors = {
            "step_halt_prob": stack_key("step_halt_probs"),
            "step_change": stack_key("step_changes"),
            "loop_scale": stack_key("loop_scales"),
            "prob_remain_max": stack_key("prob_remain_max"),
            "prob_remain_mean": stack_key("prob_remain_mean"),
        }
