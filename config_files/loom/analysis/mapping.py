# Experiment manifest for adaptive computation runs
...
# Filter functions
def get_loop3_plus_runs():
    """Get runs with loop3 or higher (excludes baseline and loop1)"""
    return [r for r in EXPERIMENT_MANIFEST 
            if not r['name'].startswith('baseline') and not r['name'].startswith('loop1')]

def get_runs_by_prefix(prefix):
    """Get runs matching a prefix"""
    return [r for r in EXPERIMENT_MANIFEST if r['name'].startswith(prefix)]

def get_runs_by_names(names):
    """Get specific runs by name"""
    return [r for r in EXPERIMENT_MANIFEST if r['name'] in names]