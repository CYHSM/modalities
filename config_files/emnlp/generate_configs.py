...
In FLOP-matched mode, every command ALSO emits a "min-deep" dual variant where
ffn_deep is pinned to the rounding floor (ffn_round_multiple). The wide path
absorbs the remaining FLOP budget. This represents the most wide-skewed dual
possible at the given budget — useful as an extreme reference point.

Subcommands:
  single             param-matched, one dual + baselines
  sweep              param-matched, sweep ffn_deep
  flop-single        FLOP-matched, one budget + baselines (+ optional dual α grid + min-deep)
...
#
#   dual at min ffn_deep (pinned to ffn_round_multiple, wide absorbs rest):
#       deep_used = max_loops * (attn + 6d * ffn_deep_min)
#       wide_target = F_arith - deep_used
#       ffn_wide = (wide_target - attn) / (6d)
#
...
def solve_flop_matched_dual_min_deep(flop_budget: int, d_model: int, max_loops: int,
                                     n_rep: int = 1,
                                     ffn_round_multiple: int = 64
                                     ) -> tuple[int, int, dict]:
    """
    Solve for a dual where ffn_deep is pinned to ffn_round_multiple (the floor)
    and ffn_wide absorbs whatever FLOP budget remains.

    This is the most wide-skewed dual that still has a non-trivial deep path
    at the given per-layer FLOP budget. Useful as an extreme reference point
    in the deep/wide tradeoff sweep.
    """
    attn = attn_proj_flops(d_model, n_rep)
    gate = gate_flops(d_model)
    router = router_flops(d_model, max_loops)
    ...
    if f_arith <= 0:
        raise ValueError(
            f"FLOP-matched dual (min-deep): budget {flop_budget} insufficient "
            f"even to cover gate+router ({gate + router})."
        )

    ffn_deep = ffn_round_multiple
    deep_used = max_loops * (attn + swiglu_flops(d_model, ffn_deep))

    if deep_used >= f_arith:
        raise ValueError(
            f"FLOP-matched dual (min-deep): even the floor ffn_deep={ffn_deep} "
            f"exhausts the budget on the deep path "
            f"(deep_used={deep_used} >= f_arith={f_arith}). Increase --flop-budget "
            f"or decrease --ffn-round-multiple."
        )
    ...
    if ffn_wide_exact <= 0:
        raise ValueError(
            f"FLOP-matched dual (min-deep): wide path can't cover its attn cost "
            f"(ffn_wide_exact={ffn_wide_exact:.0f}). Increase --flop-budget."
        )
...
def build_flop_matched_dual_min_deep(flop_budget: int, d_model: int, n_layers: int,
                                     max_loops: int,
                                     ...
                                     ffn_round_multiple: int = 64) -> dict:
    """
    Dual variant with ffn_deep pinned to the rounding floor.
    Wide path absorbs the remaining FLOP budget.
    """
    n_rep = (n_head_q // n_head_kv) if (n_head_q and n_head_kv) else 1

    ffn_deep, ffn_wide, actual_flops = solve_flop_matched_dual_min_deep(
        flop_budget=flop_budget, d_model=d_model, max_loops=max_loops,
    ...
    return {
        "kind": "dual",
        "variant": "min_deep",     # marker so make_exp_id can disambiguate
        "match_mode": "flop",
        "flop_budget_per_layer": flop_budget,
        "alpha": alpha_actual,     # report the implied alpha — there's no target here
...
def make_exp_id(cfg: dict) -> str:
    ...
    if cfg["kind"] == "dual":
        if cfg.get("variant") == "min_deep":
            # Distinct tag so it can't collide with any α-targeted dual id.
            alpha_tag = "_aMINdeep"
        elif cfg.get("match_mode") == "flop":
...
def _cfg_to_yaml_patches(cfg: dict) -> dict:
    ...
    if kind == "dual":
        ...
        # Explicit layer_types so the model never falls back to the
        # wide_ffn_hidden>0 heuristic.
        ...
        # ffn_hidden is unused in wide layers but must satisfy the size
        # validator (must be divisible by 64); 64 is the smallest legal value.
        patches[("model_raw", "ffn_hidden")] = "64"
        patches[("model_raw", "wide_ffn_hidden")] = str(cfg["ffn_wide"])
        patches[("model_raw", "max_loops")] = "1"
        patches[("model_raw", "enable_adaptive")] = "true"
        # CRITICAL: without explicit layer_types, the model would build
        # 'dual' layers (because wide_ffn_hidden > 0) instead of 'wide'.
        ...
        patches[("model_raw", "max_loops")] = str(cfg["max_loops"])
        patches[("model_raw", "enable_adaptive")] = "true"
        # sandwich already sets cfg["layer_types"] in build_mixed_sandwich.
...
def _patch_yaml(template_text: str, cfg: dict) -> str:
    ...
    for line in lines:
        ...
        # Drop any existing layer_types line in model_raw — we're going to
        # write our own below max_loops, and YAML duplicates are ugly.
...
def print_flop_matched_summary(budget: int, d_model: int, n_layers: int, max_loops: int,
    ...
    for dc in dual_cfgs:
        if dc.get("variant") == "min_deep":
            label = f"dual min-deep (α≈{dc['alpha_actual']:.2f})"
        else:
...
def _build_flop_triple(args, budget: int, alpha_list: list[float]) -> tuple[dict, dict, list[dict]]:
    ...
    Always also emits a "min-deep" dual variant (ffn_deep pinned to the rounding
    floor, wide path absorbs the rest of the budget). This is appended to the
    dual list, so every flop-* command gets it for free.
    """
    ...
    # Always also build the min-deep dual variant.
    if not getattr(args, "no_min_deep", False):
        try:
            dc_min = build_flop_matched_dual_min_deep(
                flop_budget=budget, d_model=args.d_model, n_layers=args.n_layers,
            ...
            # De-dup: if an explicitly requested α produced exactly the same
            # (ffn_deep, ffn_wide), skip the min-deep variant to avoid two
            # configs writing the same YAML — but it'll have a different
            # exp_id (the _aMINdeep tag), so collisions on disk aren't a
            # concern. We still skip on the table to keep the summary clean.
            already_present = any(
                dc["ffn_deep"] == dc_min["ffn_deep"] and dc["ffn_wide"] == dc_min["ffn_wide"]
                for dc in dual_cfgs
            )
            if not already_present:
                dual_cfgs.append(dc_min)
        except ValueError as e:
            print(f"  ⚠  Skipping dual min-deep at budget {_fmt_flops(budget)}: {e}")
...
def build_parser():
    ...
    def add_flop_matched(sp):
        sp.add_argument("--no-min-deep", action="store_true", default=False,
                        help="Disable the auto-emitted dual min-deep variant "
                             "(ffn_deep pinned to the rounding floor).")
