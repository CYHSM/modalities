...
def parse_ablation(spec: str):
    ...
    for part in spec.split(","):
        ...
        if not part:
            continue
        m_tags = re.match(r"^mask_tags\s*=\s*([A-Z+]+)$", part)
        if m_tags:
            out["mask_tags"] = sorted(list(set(m_tags.group(1).split("+"))))
            continue
        if part == "shuffle":
            out["shuffle"] = True
            continue
...
def ablation_name(spec_dict):
    ...
    if "mask_tags" in spec_dict: parts.append(f"mask_{'+'.join(sorted(spec_dict['mask_tags']))}")
...
def _wrap_convex_gate(gate_module, g_deep, g_wide, do_shuffle):
    ...
    def patched_forward(self, x, h_deep, h_wide, record=False):
        ...
        # ---- override ----
        mask = CLASS_MASK["mask"]
        if mask is not None:
            mask = mask.unsqueeze(-1)
            
        if g_deep is not None:
            tgt = _t.full_like(gate, float(g_deep))
            gate = _t.where(mask, tgt, gate) if mask is not None else tgt
        elif g_wide is not None:
            tgt = _t.full_like(gate, 1.0 - float(g_wide))
            gate = _t.where(mask, tgt, gate) if mask is not None else tgt
        elif do_shuffle:
...
def _wrap_two_gates(gate_module, g_deep, g_wide, do_shuffle):
    ...
    def patched_forward(self, x, h_deep, h_wide, record=False):
        ...
        if do_shuffle:
            ...
            if g_deep is not None:
                tgt = _t.full_like(out_gates[..., 0], float(g_deep))
                out_gates[..., 0] = _t.where(mask, tgt, out_gates[..., 0]) if mask is not None else tgt
            if g_wide is not None:
                tgt = _t.full_like(out_gates[..., 1], float(g_wide))
                out_gates[..., 1] = _t.where(mask, tgt, out_gates[..., 1]) if mask is not None else tgt
            gates = out_gates
        # ------------------
...
def eval_one_source(model, parquet_path, max_chunks, device, pos_map=None, mask_tags=None):
    ...
    for r in rows:
        ...
        if mask_tags is not None:
            if tags is not None and len(tags) == r["n_tokens"]:
                if "RANDOM" in mask_tags:
                    target_tags = [t for t in mask_tags if t != "RANDOM"]
                    if not target_tags:
                        target_tags = ["NUM", "SYM"]
                    count = sum(t in target_tags for t in tags)
                    m = torch.zeros(len(tags), dtype=torch.bool)
                    if count > 0:
                        non_target_indices = [i for i, t in enumerate(tags) if t not in target_tags]
                        if len(non_target_indices) >= count:
                            perm_indices = torch.tensor(non_target_indices)[torch.randperm(len(non_target_indices))[:count]]
                        else:
                            perm_indices = torch.tensor(non_target_indices)
                        if len(perm_indices) > 0:
                            m[perm_indices] = True
                    CLASS_MASK["mask"] = m.unsqueeze(0).to(device)
                else:
                    m = torch.tensor([[t in mask_tags for t in tags]], device=device)
                    CLASS_MASK["mask"] = m
                total_masked_tokens += int(CLASS_MASK["mask"].sum())
            else:
                CLASS_MASK["mask"] = None
        else:
            CLASS_MASK["mask"] = None
...
def main():
    ...
    for pq_path in parquets:
        ...
        print(f"\n[{source}]")
        r = eval_one_source(model, pq_path, args.max_chunks, args.device, pos_map, spec.get("mask_tags"))
